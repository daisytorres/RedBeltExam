...
//taking in newUser's password and setting it = to hasher.Hashpassword and it takes in the new object and it's updated key as the parameters
    ...
    HttpContext.Session.SetString("LoggedUserName", newUser.UserName); //since we need their name on every page
    return RedirectToAction("Dashboard", "Coupon"); //view page, controller
    }
...
//route for login user 
    ...
    public IActionResult LoginUser (LogUser logAttempt)
    {
        ...
        HttpContext.Session.SetString("LoggedUserName", dbUser.UserName);
